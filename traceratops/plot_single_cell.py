...
def plotsSubplot_sc_matrices(him_data, nRows, output="subplotMatrices.png"):
    ...
    sc_matrix, sorted_values, _ = sort_cells_by_number_pwd(him_data)

    # displays plots
    Ncells2Process = nRows**2
    cell_id, _ = returnCellsHighestNumberPWD(sorted_values, Ncells2Process)
...
def plotsRgvalues(
    ...
    min_number_pwd=6,
    threshold=6,
    bandwidths=np.linspace(-1.5, 0, 20),
):
    print("Threshold = {} px | min number PWDs = {}".format(threshold, min_number_pwd))

    sc_matrix, sorted_values, _ = sort_cells_by_number_pwd(him_data)
    Ncells2Process = nRows**2
    selectedCellsIDs, _ = returnCellsHighestNumberPWD(sorted_values, Ncells2Process)
...
def makesPlotHistograms(
    him_data, run_parameters, output_filename="./HiMhistograms.png"
):
    sc_matrix, _, _ = sort_cells_by_number_pwd(him_data)
...
def main():
    ...
    root_folder, output_folder, run_parameters = create_dict_args(args)
    ...
    _, sc_matrix = plotsSubplot_sc_matrices(him_data, nRows, output=output)
    ...
    if run_parameters["plotHistogramMatrix"]:
        ...
        makesPlotHistograms(him_data, run_parameters, output_filename=output)
