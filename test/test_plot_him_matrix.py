...
from traceratops.plot_him_matrix import parse_arguments
...
def test_parse_arguments_accepts_input_and_legacy_matrix_alias():
    parser = parse_arguments()

    canonical_args = parser.parse_args(["--input", INPUT_NPY, "--barcodes", INPUT_ECSV])
    legacy_args = parser.parse_args(["--matrix", INPUT_NPY, "--barcodes", INPUT_ECSV])

    assert canonical_args.input == INPUT_NPY
    assert legacy_args.input == INPUT_NPY
...
def test_real_data():
    ...
    nan_filename = "Fig_n_cells_250_pwd_sc_matrix_nan_0.81-0.96.png"
...
missing_args_cases = [
    ["plot_him_matrix"],
    ["plot_him_matrix", "--input", INPUT_NPY],
    ["plot_him_matrix", "-B", INPUT_ECSV],
]


@pytest.mark.parametrize(
    "bad_args", missing_args_cases, ids=["BOTH", "--barcodes", "--input"]
)
...
def test_plot_format(format):
    # Determine output file that should be generated by the script
    expected_nan_name = "Fig_bc_4_cells_4_nan_0.25-0.75"
...
def test_mode(config):
    ...
    expected_nan_name = "Fig_bc_4_cells_4_nan_" + config.split("_")[1]
...
def test_visualization_args():
    ...
    generated_nan_path = os.path.join(INPUT_DIR, "Fig_bc_4_cells_4_nan_0.25-0.75.png")
...
def test_norm():
    # Determine output file that should be generated by the script
    expected_nan_name = "Fig_bc_4_cells_4_nan_norm"
    generated_nan_path = os.path.join(INPUT_DIR, f"{expected_nan_name}_0.25-0.75.png")
...
def test_matplotlib_v3_5_1_error():
    ...
    nan_filename = "Fig_n_cells_250_pwd_sc_matrix_nan_norm_0.81-0.96.png"
    generated_png_path = os.path.join(INPUT_DIR, png_filename)
    generated_nan_path = os.path.join(INPUT_DIR, nan_filename)
    generated_matrix_path = os.path.join(INPUT_DIR, matrix_filename)
    expected_matrix_path = os.path.join(OUTPUT_DIR, matrix_filename)
